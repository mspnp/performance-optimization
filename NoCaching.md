...
- Examining data access statistics and other information provided by a data store might show the same queries being repeated frequently. For example, Microsoft SQL Server provides the `sys.dm_exec_query_stats` management view which contains statistical information for recently executed queries. The text for each query is available in the `sys.dm_exec-query_plan` view. You can use the following SQL query to determine the frequency with which queries are performed:
...
In the CachingDemo sample application, the following query corresponds to the request generated by the Entity Framework code shown earlier:
...
- Monitoring requests may indicate that the same data is being retrieved repeatedly. Using an application performance monitor you can examine which resources are accessed most frequently. For example, if you configure the CachingDemo to capture monitoring data by using [Microsoft AppInsights][AppInsights], the analytics generated can quickly show you the frequency with which each server request occurs. The following image shows the analytics captured for the CachingDemo application. Three different HTTP GET operations (`Person/Get`, `Person/GetAsync`, and `Person/GetCached`) have been performed while the application has been running: 

![AppInsights showing server requests for the CachingDemo application][AppInsights-server-requests]

In this example, `Person\Get` accounts for 68.8% of all requests. You can drill into this request to obtain more information and view the specific instances for this request, including the full URL which identifies the resource being retrieved. A useful feature here is the ability to filter the data. The `URL base` filter displays a count of the number of times each resource has been accessed. In the following image, each resource has been retrieved at least 190 times. The data is filtered by using the URL base `/api/Person/Get/10`, and the Diagnostic search pane displays the details for each access, including the time the access was made. If these accesses occur within a relatively short period of time, then the data returned by accessing this resource is a good candidate for caching. Note that this data may be an entity, or it could be the results of a computation:
...
- [Microsoft AppInsights][AppInsights]
